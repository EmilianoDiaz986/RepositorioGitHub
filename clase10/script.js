window.addEventListener('DOMContentLoaded', (event) => {
  ...
  function handleSubmit(event) {
    ...
    if (isValid) {
      ...
      function showModal(message, data) {
        ...
        // Mostrar el modal en el documento
        document.body.appendChild(modal);
    
         // Guardar los datos recibidos en LocalStorage si la suscripción fue exitosa
      if (data && data.success) {
        localStorage.setItem('newsletterData', JSON.stringify(data));
      }


        // Realizar llamada HTTP para enviar los datos del formulario
    fetch('http://curso-dev-2021.herokuapp.com/newsletter', {
      ...
      .then((data) => {
        // Mostrar mensaje emergente con la información recibida
        const message = data.success
          ? `Suscripción exitosa: ${data.message}`
          : `Error en la suscripción: ${data.message}`;
      ...
      .finally(() => {
        // Reiniciar el formulario y actualizar el título
        event.target.reset();
        updateFormTitle();
  ...
  // Ejecutar la función para cargar los datos guardados en LocalStorage
  loadNewsletterData();
